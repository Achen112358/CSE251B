...
def createDeepLabv3(outChannel=10):
...
model_name = 'transfer_leaning_model_p5b.pt'

criterion = nn.CrossEntropyLoss() # Choose an appropriate loss function from https://pytorch.org/docs/stable/_modules/torch/nn/modules/loss.html
n_class = 10
fcn_model = createDeepLabv3()
...
def train(epochs = 20, learning_rate=0.0001):
    ...
    for epoch in range(epochs):
        ...
        for iter, (inputs, labels) in enumerate(train_loader):
            ...
            outputs = fcn_model(inputs)['out'] #we will not need to transfer the output, it will be automatically in the same device as the model's!
...
def val(epoch):
    ...
    with torch.no_grad(): # we don't need to calculate the gradient in the validation/testing
        ...
        for iter, (input_, label) in enumerate(val_loader):
